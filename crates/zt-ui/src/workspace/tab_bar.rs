...
impl Workspace {
    ...
    /// Build the tab bar strip (TITLEBAR_H tall) for the PDF content area.
    ///
    /// Visually identical to `render_tab_bar` but operates on `pdf_tabs`/
    /// `active_pdf_idx`/`pending_pdf_close` instead of the notes equivalents.
    pub(super) fn render_pdf_tab_bar(&mut self, cx: &mut Context<Self>) -> AnyElement {
        let text_color = theme::text();
        let subtext = theme::subtext0();
        let surface0 = theme::surface0();
        ...
        // Push tabs safely past the macOS traffic lights when the left sidebar is hidden.
        let tab_left_pad = if self.left_visible { 4.0f32 } else { 76.0f32 };
        let active_idx = self.active_pdf_idx;
        ...
        let mut tab_elements: Vec<AnyElement> = self
            ...
            .map(|(i, tab)| {
                let is_active = i == active_idx;
                let tab_id = SharedString::from(format!("pdf-tab-{}", i));
                let close_id = SharedString::from(format!("pdf-tab-close-{}", i));
                ...
                let tab_pill = div()
                    ...
                    .text_color(if is_active { text_color } else { subtext })
                    .on_click(cx.listener(move |workspace, _: &ClickEvent, _, cx| {
                        if let Some(close_idx) = workspace.pending_pdf_close.take() {
                            workspace.close_pdf_tab(close_idx, cx);
                        } else {
                            workspace.active_pdf_idx = i;
                            cx.notify();
                        }
                    }))
                    .child(
                    ...
                    .child(
                        div()
                            ...
                            .on_mouse_down(
                                MouseButton::Left,
                                cx.listener(move |workspace, _: &MouseDownEvent, _, _| {
                                    workspace.pending_pdf_close = Some(i);
                                }),
                            )
                            .child("×"),
                    );
        ...
        // ＋ new-file button at the right end of the tab bar
        tab_elements.push(
            div()
                .id("pdf-tab-new")
                .flex()
                ...
                .hover(|s| s.bg(theme::surface0()).text_color(text_color))
                .on_click(cx.listener(|workspace, _: &ClickEvent, window, cx| {
                    if let Some(ref root) = workspace.vault_root.clone() {
                        let used: std::collections::HashSet<String> =
                            workspace.pdf_tabs.iter().map(|t| t.rel_path.to_string()).collect();
                        let name = (1usize..)
                            .map(|i| format!("untitled-{}.typ", i))
                            .find(|n| !root.join(n).exists() && !used.contains(n))
                            .unwrap_or_else(|| "untitled.typ".to_string());
                        if let Some(rel) = file_ops::create_file(root, "", &name).log_err("Create file") {
                            workspace.open_pdf_file(&rel, window, cx);
                        }
                    }
                }))
                .child("+")
